...
import { updateHostel } from '@/actions/hostels';
import { useRouter } from 'next/navigation';

export default function EditHostelModal({ hostel, onClose }: { hostel: any; onClose: () => void }) {
  const [name, setName] = useState(hostel.HostelName);
  const [gender, setGender] = useState(hostel.GenderType);
  const [city, setCity] = useState(hostel.City || 'Lahore');
  const [address, setAddress] = useState(hostel.Address || '');
  const [hasWifi, setHasWifi] = useState(!!hostel.HasWifi);
  const [hasAC, setHasAC] = useState(!!hostel.HasAC);
  const [hasLaundry, setHasLaundry] = useState(!!hostel.HasLaundry);
  const [hasFood, setHasFood] = useState(!!hostel.HasFood);
  
  const [wifiPrice, setWifiPrice] = useState(hostel.WifiPrice?.toString() || '0');
  const [acPrice, setAcPrice] = useState(hostel.ACPrice?.toString() || '0');
  const [laundryPrice, setLaundryPrice] = useState(hostel.LaundryPrice?.toString() || '0');
  const [foodPrice, setFoodPrice] = useState(hostel.FoodPrice?.toString() || '0');

  const [monthlyRent, setMonthlyRent] = useState(hostel.MonthlyRent?.toString() || '5000');
  const [includedServices, setIncludedServices] = useState(hostel.IncludedServices || 'Security, Water, Electricity');
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-[#020617]/80 backdrop-blur-sm overflow-y-auto">
      <div className="w-full max-w-lg bg-slate-900 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 my-8">
        <header className="p-8 border-b border-white/5 bg-slate-950/40">
          <h2 className="text-2xl font-black text-white">Edit Hostel</h2>
          <p className="text-slate-400 text-sm font-medium">Update your property pricing, food options, and amenity details.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5 max-h-[75vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Baseline Rent (PKR)</label>
              <input
                type="number"
                required
                value={monthlyRent}
